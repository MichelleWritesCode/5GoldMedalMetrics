...
const bestOfSomething = (something, country) => {
  if (something === 'year' || something === 'discipline' || something === 'sport' || something === 'event') {
    return `SELECT ${something}, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}' GROUP BY ${something} ORDER BY COUNT(*) DESC LIMIT 1;`;
  }
  return null;
}
...
const bestYear = country => {
    return bestOfSomething('year', country);
};
...
const bestDiscipline = country => {
  return bestOfSomething('discipline', country);
};
...
const bestSport = country => {
  return bestOfSomething('sport', country);
};
...
const bestEvent = country => {
  return bestOfSomething('event', country);
};

const genderMedalists = (gender, country) => {
  if (gender === 'Men' || gender === 'Women') {
    return `SELECT gender, COUNT(DISTINCT name) AS count FROM GoldMedal WHERE country = '${country}' AND gender = '${gender}';`;
  }
  return null;
}
